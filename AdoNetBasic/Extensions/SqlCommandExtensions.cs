...
internal static class SqlCommandExtensions
{
    ...
    public static Task<int> ExecuteWriteCommandAsync(this SqlCommand command, SqlParameter parameter)
        => command.ExecuteWriteCommandAsync(new SqlParameter[] { parameter });

    public static async Task<int> ExecuteWriteCommandAsync(this SqlCommand command, SqlParameter[] parameters)
    {
        using SqlConnection connection = new(Constants.ConnectionString);
        await connection.OpenAsync();

        return await command.ExecuteWriteCommandAsync(connection, parameters);
    }

    public static async Task<int> ExecuteWriteCommandAsync(this SqlCommand command, IReadOnlyList<SqlParameter[]> parametersSets)
    {
        using SqlConnection connection = new(Constants.ConnectionString);
        await connection.OpenAsync();
        ...
        try
        {
            ...
            foreach (SqlParameter[] parameters in parametersSets)
                rowsAffected += await command.ExecuteWriteCommandAsync(connection, parameters);
    ...
    private static Task<int> ExecuteWriteCommandAsync(this SqlCommand command, SqlConnection connection, SqlParameter[] parameters)
    {
        command.Connection ??= connection;
